        ...
        {order && order.map((item, index) => (
          <div key={index} className='bg-slate-300 p-4 rounded-md shadow-md'>
            ...
            <div className='w-full flex items-center justify-center mt-5'>
              <Button className={`rounded-xl bg-blue-400 text-white disabled:cursor-not-allowed `} disabled={item?.userCancel||false} onClick={()=>handleUserCancel(item?._id)}>User Cancel</Button>
            </div>
            <div className='w-full flex items-center justify-center mt-5'>
              <Button className='rounded-xl bg-blue-400 text-white ' disabled={item?.freelancerAproved||false} onClick={()=>handleFreelancerCancel(item?._id)}>Freelancer Cancel</Button>
            </div>
            <div className='w-full flex items-center justify-center mt-5'>
                <Button className='rounded-xl bg-blue-400 text-white ' disabled={item?.admineApproved||false} onClick={()=>handleApprove(item?._id)}>Approve the Order</Button>
            </div>
            </div>
          ))}
      </div>
